...
#since the new infile has the topmed ids, we don't need the url_dict
...
out_file.write('variable_phv	tag_id	ncit	hpo	mondo	efo	oba	variable	data_set	study\n')
...
for line in infile: #iterate over infile
	...
	variable_acc = row[3]
	set_acc = row[4]
	#m = re.match('\w\w\w[\d]+', set_acc) #only need to manipulate set_acc if using urls
	#set_id = m[0][-4:]
	study_acc = row[5]
	
	#not using the urls, so we don't need these
	...
	for line in curies: #look for phenotype in ontology map
		...
		if topmed_id == tag_id: #when phenotype found, grab ontology terms
			...
			efo = row[13]
			oba = row[16]
			out_file.write('\t'.join([variable_phv,tag_id,ncit,hpo,mondo,efo,oba,variable_acc,set_acc,study_acc]) + '\n') #output mapping
		else:
			continue